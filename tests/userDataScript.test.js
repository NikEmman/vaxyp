...
describe("applyTheme (DOM)", () => {
  it.skip("adds dark class for dark theme", () => {
    document.body.className = "";
    document.body.classList.add("light");

    document.body.dispatchEvent(new window.CustomEvent("themechange", { detail: "dark" }));
  ...
  it.skip("removes dark class for light theme", () => {
...
describe("addAnakritikoi (DOM)", () => {
  it.skip("creates row with correct structure", () => {
    const container = document.getElementById("anakritikoiList");
    const initialCount = container.querySelectorAll(".anakritikoi-row").length;

    document.getElementById("addAnakritikoi").click();

    const rows = container.querySelectorAll(".anakritikoi-row");
    expect(rows.length).toBe(initialCount + 1);

    const newRow = rows[rows.length - 1];
    expect(newRow.className).toBe("anakritikoi-row");
    expect(newRow.querySelector(".row-label")).toBeTruthy();
    expect(newRow.querySelector(".row-fields")).toBeTruthy();
    expect(newRow.querySelector("select[name='anakrSex[]']")).toBeTruthy();
    expect(newRow.querySelector("input[name='anakritikoi[]']")).toBeTruthy();
    expect(newRow.querySelector("input[name='anakritikoiEnikos[]']")).toBeTruthy();
  });

  it("sets values on new row", () => {
    document.getElementById("addAnakritikoi").click();

    const rows = document.querySelectorAll(".anakritikoi-row");
    const newRow = rows[rows.length - 1];

    expect(newRow.querySelector("input[name='anakritikoi[]']").value).toBe("");
    expect(newRow.querySelector("input[name='anakritikoiEnikos[]']").value).toBe("");
    expect(newRow.querySelector("select[name='anakrSex[]']").value).toBe("Άντρας");
  });
});

describe("updateRowLabels (DOM)", () => {
  it.skip("updates labels after adding rows", () => {
    document.getElementById("addAnakritikoi").click();
    document.getElementById("addAnakritikoi").click();

    const rows = document.querySelectorAll(".anakritikoi-row");
    expect(rows[0].querySelector(".row-label").textContent).toBe("Ανακριτικός #1");
    expect(rows[1].querySelector(".row-label").textContent).toBe("Ανακριτικός #2");
    expect(rows[2].querySelector(".row-label").textContent).toBe("Ανακριτικός #3");
  });
});

describe("Remove button (DOM)", () => {
  it.skip("removes last row when more than one exists", () => {
    document.getElementById("addAnakritikoi").click();
    document.getElementById("addAnakritikoi").click();
    ...
    document.getElementById("removeAnakritikoi").click();

    const rows = container.querySelectorAll(".anakritikoi-row");
    expect(rows.length).toBe(initialCount - 1);
  });

  it("does not remove when only one row exists", () => {
    const container = document.getElementById("anakritikoiList");
    const initialCount = container.querySelectorAll(".anakritikoi-row").length;
